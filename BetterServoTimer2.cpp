...
ISR (TIMER2_OVF_vect) {
	...
	else if(ISRCount > servos[Channel].counter) { // we have finished timing the channel so pulse it low and move on
...
BetterServoTimer2::BetterServoTimer2() {
...
uint8_t BetterServoTimer2::attach(int pin) {

	return attach(pin, MIN_PULSE_WIDTH, MAX_PULSE_WIDTH);
}

uint8_t BetterServoTimer2::attach(int pin, int minPulseWidth, int maxPulseWidth) {
...
boolean BetterServoTimer2::attached() {
...
static void writeChan(uint8_t chan, int pulsewidth) {
	...
	if((chan > 0) && (chan <= NBR_CHANNELS)) { // ensure channel is valid
		...
		if(pulsewidth < servos[chan].minPulseWidth) { // ensure pulse width is valid

			pulsewidth = servos[chan].minPulseWidth;
		}
		else if(pulsewidth > servos[chan].maxPulseWidth) {

			pulsewidth = servos[chan].maxPulseWidth;	 
		}
